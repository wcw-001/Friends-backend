...
import com.wcw.usercenter.mapper.ChatMapper;
...
import com.wcw.usercenter.model.vo.WebSocketVO;
...
import static com.wcw.usercenter.contant.ChatConstant.*;
...
public class ChatServiceImpl extends ServiceImpl<ChatMapper, Chat>
    ...
    public List<ChatMessageVO> getPrivateChat(ChatRequest chatRequest, int chatType, User loginUser) {
        ...
        chatLambdaQueryWrapper
                ...
                .or(privateChat -> privateChat.eq(Chat::getToId, loginUser.getId()).eq(Chat::getFromId, toId))
    ...
    public void saveCache(String redisKey, String id, List<ChatMessageVO> chatMessageVOS) {
        try {
            ...
            if (redisKey.equals(CACHE_CHAT_HALL)) {
                valueOperations.set(redisKey, chatMessageVOS, 2 + i / 10, TimeUnit.HOURS);
            } else {
                valueOperations.set(redisKey + id, chatMessageVOS, 2 + i / 10, TimeUnit.HOURS);
            }
        } catch (Exception e) {
    ...
    private List<ChatMessageVO> returnMessage(User loginUser, Long userId, LambdaQueryWrapper<Chat> chatLambdaQueryWrapper) {
        ...
        return chatList.stream().map(chat -> {
            ...
            if (userService.getById(chat.getFromId()).getUserRole() == ADMIN_RILE || isCaptain) {
